...
func parseInput(in string) []*Sequence {
	var seq []*Sequence
	...
	for _, line := range input {
		...
		for i, signal := range signals {
			dis[i] = &Display{
				Segments: []byte(signal),
				Length:   len(signal),
				Unique:   len(signal) == 2 || len(signal) == 4 || len(signal) == 3 || len(signal) == 7,
			}
		}
		var out [4]*Display
		for i, output := range outputs {
			out[i] = &Display{
				Segments: []byte(output),
				Length:   len(output),
				Unique:   len(output) == 2 || len(output) == 4 || len(output) == 3 || len(output) == 7,
			}
		}
		seq = append(seq, &Sequence{
			Signal: dis,
			Output: out,
		})
	}
...
func Part2(input string) int {
	...
	for _, line := range lines {
		for _, perm := range perms {
			validPerm := true
			for _, signal := range line.Signal {
				possible := perm.PossibleNums(signal.Segments)
				if len(possible) == 0 {
					validPerm = false
					break
				}
			...
			if validPerm {
				// fmt.Printf("Found valid permutation: %s\n", perm.perm)
				total += score(perm, line)
			}
		}
	}
	return total
...
func score(perm *Permutation, seq *Sequence) int {
	var total string
	for _, display := range seq.Output {
		possible := perm.PossibleNums(display.Segments)
		if len(possible) != 1 {
			log.Fatalf("Something went wrong: %v, %q, %v", possible, perm.perm, seq)
		}
		total += fmt.Sprintf("%d", possible[0])
	}
	totalint, _ := strconv.Atoi(total)
	return totalint
}

func generatePermutations() []*Permutation {
	var res []string
	baseString := "abcdefg"
	for _, c1 := range baseString {
		for _, c2 := range baseString {
			if c2 == c1 {
				continue
			}
			for _, c3 := range baseString {
				if c3 == c1 || c3 == c2 {
					continue
				}
				for _, c4 := range baseString {
					if c4 == c1 || c4 == c2 || c4 == c3 {
						continue
					}
					for _, c5 := range baseString {
						if c5 == c1 || c5 == c2 || c5 == c3 || c5 == c4 {
							continue
						}
						for _, c6 := range baseString {
							if c6 == c1 || c6 == c2 || c6 == c3 || c6 == c4 || c6 == c5 {
								continue
							}
							for _, c7 := range baseString {
								if c7 == c1 || c7 == c2 || c7 == c3 || c7 == c4 || c7 == c5 || c7 == c6 {
									continue
								}
								res = append(res, fmt.Sprintf("%c%c%c%c%c%c%c", c1, c2, c3, c4, c5, c6, c7))
							}
						}
					}
				}
	...
	perms := make([]*Permutation, len(res))
	for i, r := range res {
		perms[i] = &Permutation{
			perm: r,
		}
		perms[i].GenerateNumToChars()
	}
...
type Permutation struct {
	perm       string
	NumToChars map[int][]byte
}

func (p *Permutation) GenerateNumToChars() {
	p.NumToChars = make(map[int][]byte)
	for i := range 10 {
		switch i {
		case 0:
			p.NumToChars[0] = []byte{p.perm[0], p.perm[1], p.perm[2], p.perm[4], p.perm[5], p.perm[6]}
		case 1:
			p.NumToChars[1] = []byte{p.perm[2], p.perm[5]}
		case 2:
			p.NumToChars[2] = []byte{p.perm[0], p.perm[2], p.perm[3], p.perm[4], p.perm[6]}
		case 3:
			p.NumToChars[3] = []byte{p.perm[0], p.perm[2], p.perm[3], p.perm[5], p.perm[6]}
		case 4:
			p.NumToChars[4] = []byte{p.perm[1], p.perm[2], p.perm[3], p.perm[5]}
		case 5:
			p.NumToChars[5] = []byte{p.perm[0], p.perm[1], p.perm[3], p.perm[5], p.perm[6]}
		case 6:
			p.NumToChars[6] = []byte{p.perm[0], p.perm[1], p.perm[3], p.perm[4], p.perm[5], p.perm[6]}
		case 7:
			p.NumToChars[7] = []byte{p.perm[0], p.perm[2], p.perm[5]}
		case 8:
			p.NumToChars[8] = []byte{p.perm[0], p.perm[1], p.perm[2], p.perm[3], p.perm[4], p.perm[5], p.perm[6]}
		case 9:
			p.NumToChars[9] = []byte{p.perm[0], p.perm[1], p.perm[2], p.perm[3], p.perm[5], p.perm[6]}
		}
		sort.Slice(p.NumToChars[i], func(a, b int) bool { return p.NumToChars[i][a] < p.NumToChars[i][b] })
	}
}

func (p *Permutation) PossibleNums(lights []byte) []int {
	var possible []int
	sort.Slice(lights, func(a, b int) bool { return lights[a] < lights[b] })
	for k, chars := range p.NumToChars {
		if !slices.Equal(lights, chars) {
			continue
		}
...
var realInput = `dfgabce cadfgb cefa ca aecbg dfcegb geabd ecbfg cab agcfbe | egbfadc dbgae gcfeb abgdfc
